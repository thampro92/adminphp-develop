...
//            $data = $this->BanCaLog_model->get_log_lode($this->input->post("nn"), $this->input->post("ft"), $this->input->post("et"));
            if ($this->input->is_ajax_request()) {
                $this->load->model("admin/loggamethirdparty_model");
                $rs = $this->loggamethirdparty_model->getLode($fromDate, $toDate, $nickname, $pages, $size);
                echo json_encode($rs);
            } else {
                echo "Bạn không được hack";
            }
...
//                '&fd=' . urlencode($fromDate) . '&td=' . urlencode($toDate) . '&page=' . $pages . '&page_size=' . $page_size . '&nn=' . $nn, '8090');
            if ($this->input->is_ajax_request()) {
                $this->load->model("admin/loggamethirdparty_model");
                $rs = $this->loggamethirdparty_model->lodest($fromDate, $toDate, $nn, $pages, $page_size);
                echo json_encode($rs);
            } else {
                echo "Bạn không được hack";
            }
